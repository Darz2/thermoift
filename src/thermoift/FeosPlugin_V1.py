...
def row_append(
    ...
    Optionally pads with zeros for all available components from KIJ registry
    ...
    ML_mode : bool, optional
        If True (default), pad with all available components from KIJ registry for ML training.
        If False, include only active components.

    Returns
    -------
    dict
        Results dictionary. Includes all components (with zeros for inactive)
        if ML_mode=True; only active components if ML_mode=False.
    """
    row = {}

    # Determine which components to include in output
    if ML_mode:
        all_components = list(_REGISTRY.keys())
        # To sort them alpahbatically
        # all_components = sorted(_REGISTRY.keys())
    else:
