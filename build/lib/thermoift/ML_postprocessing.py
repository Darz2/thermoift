...
class MLPostprocessing:
    ...
    def plot_residual_distribution(
        ...
        overlay_keys = [k for k in ("test", "val") if self._datasets is not None and k in self._datasets]
        ...
        if overlay_keys:
            # Common bin edges across overlaid splits so the histograms are comparable
            all_resids = np.concatenate([
                np.asarray(self._datasets[k][0]) - np.asarray(self._datasets[k][1])
                for k in overlay_keys
            ])
            bin_edges = np.histogram_bin_edges(all_resids, bins=bins)
            ...
            for key in overlay_keys:
                ...
                stat_lines.append(
                    rf"{split_labels[key]}: $\mu$={resids.mean():.2f}, "
                    rf"$\sigma$={resids.std():.2f}, RMSE={rmse:.2f} / {unit}"
                )
            textstr = "\n".join(stat_lines)
        else:
            ax.hist(
    ...
    def plot_residual_vs_predicted(
        ...
        a visual calibration band. Optional cross-validation annotation
        is placed in the upper-right corner.

        If test and validation datasets are available, two separate legends
        are shown:
        1. Test/Validation marker legend with round markers in two columns.
        2. Separate RMSE legend for the ±RMSE bands.
        ...
        cv_mae : float, optional
            Mean cross-validation MAE to annotate.
        title : str, optional
            Figure title.
        ...
        overlay_keys = [
            k for k in ("test", "val")
            if self._datasets is not None and k in self._datasets
        ]
        ...
        if overlay_keys:
            ...
            for key in overlay_keys:
                ...
                rmse_k = float(np.sqrt(mean_squared_error(y_t, y_p)))
                ...
                # Legend 1 handle: Test / Validation round marker
